	...
	else
	{
		for (int i = std::min(combo_cnt, grp_cnts[0]); i >= 0; --i)
		{
			...
			if (i == 0)
				...
				comb_0 = combination_nr_que_seq(grp_cnts[0], i);
			for (auto c : comb_0)
			{
...
int main(void)
{
	...
	auto combo = combination_obj(data, sizeof(data) / sizeof(data[0]), sizeof(data) / sizeof(data[0]), true);
	auto permu = permutation(data, sizeof(data) / sizeof(data[0]), sizeof(data) / sizeof(data[0]), true);
	...
	int mc[2] = { 3, 3 };
	std::vector<std::vector<int>> combo2 = get_multi_comb_cnt(mc, 2, 2);
	std::vector<std::vector<int>> combo1 = get_multi_comb_cnt(mc, 2, 1);
	...
	int cnts_arr[] = {3, 2, 1};
	std::vector<std::vector<std::vector<int>> > ppls = get_multi_comb_idx(cnts_arr, 3, 3);
	for (auto vvi : ppls)
	{
		for (auto vi : vvi)
		{
	...
	auto c1 = combination_obj(dt1, sizeof(dt1) / sizeof(dt1[0]), 3, true);
	auto c2 = combination_seq(sizeof(dt1) / sizeof(dt1[0]), 3, 0, true);
	auto c3 = combination_nr_que_obj(dt2, sizeof(dt2) / sizeof(dt2[0]), 3);
	auto c4 = combination_nr_stk_obj(dt2, sizeof(dt2) / sizeof(dt2[0]), 3);
	auto c5 = combination_nr_que_seq(sizeof(dt2) / sizeof(dt2[0]), 3);
	auto c6 = combination_nr_stk_seq(sizeof(dt2) / sizeof(dt2[0]), 3);

	//auto c1 = permutation_obj(dt1, sizeof(dt1) / sizeof(dt1[0]), 2);
	//auto c2 = permutation_seq(sizeof(dt1) / sizeof(dt1[0]), 2, 0);
	//auto c3 = permutation_nr_que_obj(dt2, sizeof(dt2) / sizeof(dt2[0]), 2);
	//auto c4 = permutation_nr_stk_obj(dt2, sizeof(dt2) / sizeof(dt2[0]), 2);
	//auto c5 = permutation_nr_que_seq(sizeof(dt2) / sizeof(dt2[0]), 2);
	//auto c6 = permutation_nr_stk_seq(sizeof(dt2) / sizeof(dt2[0]), 2);
